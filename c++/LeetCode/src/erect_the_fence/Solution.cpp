...
bool erect_the_fence::Solution::isClockwiseTurn(vector<int> &a, vector<int> &b, vector<int> &c) {
...
vector<vector<int>> erect_the_fence::Solution::outerTrees(vector<vector<int>> &trees) {
...
void erect_the_fence::Solution::test() {
    vector<vector<int>> points {{1, 1}, {2, 2}, {2, 0}, {2, 4}, {3, 3}, {4, 2}};
    vector<vector<int>> answer {{1, 1}, {2, 0}, {2, 4}, {3, 3}, {4, 2}};
    vector<vector<int>> result = outerTrees(points);
    for (int i = 0; i < answer.size(); i++) {
        assert(answer[i][0] == result[i][0]);
        assert(answer[i][1] == result[i][1]);
    }

    cout << "outerTrees completed!" << endl;
}
